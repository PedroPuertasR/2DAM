...
public class HiloServidorChat extends Thread {
    ...
    /*Este es el constructor de la clase HiloServidorChat, que toma dos argumentos: un objeto Socket y un objeto ComunHilos. 
    El constructor inicializa las variables de instancia socket y comunHilo y crea un objeto DataInputStream a partir 
    ...
    /*Este es el método run, que se ejecuta cuando se inicia el hilo. El método comienza imprimiendo el número actual 
    de conexiones en la consola y luego llama al método EnviarMensajesaTodos para enviar los mensajes a todos los 
    clientes conectados. Luego, el método entra en un ciclo while que se ejecuta mientras la conexión está activa. 
    Dentro del ciclo, el método lee el mensaje entrante del cliente, lo procesa y lo envía a todos los clientes conectados.
    Si se recibe el carácter "*", el método disminuye el número de conexiones activas y sale del ciclo. Finalmente*/
    public void run() {
    ...
    private void EnviarMensajesaTodos(String texto) {
        ...
        for (i = 0; i < comunHilo.getNumConexiones(); i++) {
            Socket s1 = comunHilo.getElementoTabla(i);
            if (!s1.isClosed()) {
                try {
